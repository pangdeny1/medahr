...
if (isset($_POST['submit'])) {
   exit("Contact Administrator...");
	...
	$sql = "SELECT counterindex,payrollid,employeeid,grosspay,loandeduction,sss,hdmf,philhealth,tax,heslb,otherdeduction
	...
	if(DB_num_rows($PayDetails)>0)
	{
		...
		{	
				$NetPay=$myrow['grosspay']-$myrow['loandeduction']-$myrow['sss']-$myrow['hdmf']-$myrow['philhealth']-$myrow['tax'] -$myrow['heslb']-$myrow['otherdeduction'];
				$sql = 'UPDATE prlpayrolltrans SET netpay='.$NetPay.'
						WHERE counterindex = ' . $myrow['counterindex'];
				$PostNPay = DB_query($sql,$db);
		}	 
	}
echo"Payroll has been processed Successfully";
}
?>