...
if (isset($_POST['submit'])) {
   exit("Contact Administrator...");
	...
	if ($NumberofPayday>=12) {  //payroll for monthly to daily based on frequency of payday
		...
		$NumPaydayPerMos=$NumberofPayday;
				...
				$sql = "SELECT sum(taxableincome) AS Gross,sum(tax) AS Tax
					...
					$MyEstTax=GetMyTax($MyEstTaxableIncome, $db);
							if ($UnPaidPDthisYR==0) {
								...
								$MyTaxWithheld=GetMyTax($Ctaxable, $db);
								
							} else {
							//de m m
								//$MyTaxWithheld=($MyEstTax-$TaxUpToDate)/($UnPaidPDthisYR+1);
								$MyTaxWithheld=$Ctaxable;
								...
								//Updating taxable amount
								$sql2 = 'UPDATE prlpayrolltrans SET taxableincome='.$Ctaxable.'
						                 WHERE counterindex = ' . $myrow['counterindex'];
								$PostTaxPay2 = DB_query($sql2,$db);
					if ($Ctaxable>0) {
				...
				if ($myrow['othincome']>0) {
							$sql = "SELECT amount,otherincid
								...
								{				
									while ($othrow = DB_fetch_array($OIDetails))
									{
										$OIIDDesc= GetOthIncRow($othrow['otherincid'], $db,1);
										if ($OIIDDesc=='Non-Tax') {
											$Ctaxable-=$othrow['amount'];
										}
									}
