...
$isPhilValue=1;
$DeductPH = GetYesNoStr($payroll->deductphilhealth);
...
if (isset($_POST['submit'])) {
   exit("Contact Administrator...");
	...
	if ($DeductPH=='Yes') {
		$sql = "SELECT counterindex,payrollid,employeeid,basicpay,grosspay,absent,late,otpay,fsmonth,fsyear
		...
		if(DB_num_rows($PayDetails)>0)
		{
			...
			{	
				/*$sql = "SELECT sum(grosspay) AS Gross
					...
					FROM prlpayrolltrans
					WHERE prlpayrolltrans.employeeid='" . $myrow['employeeid'] . "'
					AND prlpayrolltrans.payrollid='" . $PayrollID . "'
					AND prlpayrolltrans.fsyear='" . $FSYearRow . "'";
									 ...
									 $myphrow = GetPHRow($PHGP, $db);
										$sql = "INSERT INTO prlempphfile (		
												...
												VALUES ('$PayrollID', 
													...
													'". (($myphrow['employerph'] * $PHGP )/100)."',
													
													'". (($myphrow['employerec']* $PHGP )/100)."',
													'". (($myphrow['employeeph']* $PHGP )/100)."',
													'". (($myphrow['total'] * $PHGP )/100)."',
													'" . $myrow['fsmonth'] . "',
													'" . $myrow['fsyear'] . "'
													)";
