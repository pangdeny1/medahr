...
if (isset($_POST['submit'])) {
   exit("Contact Administrator...");
	...
	$sql="DELETE FROM prlheslbdeduction WHERE payrollid ='" . $PayrollID . "'";
	...
	$sql = "SELECT counterindex,payrollid,employeeid,heslb,basicpay
	...
	if(DB_num_rows($PayDetails)>0)
	{
		...
		{	
			$sql = "SELECT loanfileid,loantableid,employeeid,amortization,startdeduction,loanbalance
					FROM prlheslbfile
			        WHERE prlheslbfile.employeeid='" . $myrow['employeeid'] . "'
						...
						{		
							if ($loanrow['loanbalance']>0) {
								if ($loanrow['loanbalance']<=$loanrow['amortization']) {
									$sql = "INSERT INTO prlheslbdeduction(employeeid,loantableid,amount)
										SELECT employeeid,loantableid,loanbalance
										FROM prlheslbfile
										WHERE prlheslbfile.loanfileid='" . $loanrow['loanfileid'] . "'"; 
							...
							$sql = "UPDATE prlheslbdeduction SET
									payrollid='" . $PayrollID . "'
									WHERE payrollid = ''";
	...
	if(DB_num_rows($PayDetails)>0)
	{
		while ($myrow = DB_fetch_array($PayDetails))
		{
			$sql = "SELECT sum(amount) AS loanded
				FROM prlheslbdeduction
				WHERE prlheslbdeduction.employeeid='" . $myrow['employeeid'] . "'
				...
				//$otrow=DB_fetch_array($OTDetails);
				   ...
				   if ($LoanPayment>0 or $LoanPayment<>null) {
					$sql = 'UPDATE prlpayrolltrans SET heslb='.$LoanPayment.'
					     WHERE counterindex = ' . $myrow['counterindex'];
