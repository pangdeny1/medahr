   ...
   exit("Contact Administrator...");
					...
					{				
						while ($rtrow = DB_fetch_array($RTDetails))
						{
							if (($rtrow['payrollid']==$PayrollID) or ($rtrow['payrollid']=='')){
									$PayType=GetPayTypeDesc(GetPayType($rtrow['employeeid'], $db));
	...
	if(DB_num_rows($PayDetails)>0)
	{
		...
		{	
			...
			$PayType=GetPayTypeDesc(GetPayType($myrow['employeeid'], $db));
			if ($PayType=='Hourly') {
				$sql = "SELECT sum(regamt) AS BasicPay,employeeid,rtdate
					FROM prldailytrans
					WHERE prldailytrans.employeeid='" . $myrow['employeeid'] . "'
